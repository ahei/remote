...
执行完以上操作后, 该集群里面的每台机器之间即可互通, 不用输入密码就可以ssh登录
**如果对安全比较重视的话, 需要慎重** , 不过可以把少量的一些机器当做一个集群, 在这个集群内做到互通
...
环境变量
------
* SSH 如果你想使用其他工具代替ssh, 可以设置该环境变量. 有些公司有自己的权限管理系统, 内部机器之间登录并不一定使用ssh, 可以设置该环境变量.
* REMOTE_GET_HOSTS 如果你不想使用配置文件来获取集群内的机器列表, 比如从zookeeper上获取机器列表, 可以设置该环境变量为你的脚本文件名, 你的脚本文件参数为集群名, 需要把该集群的机器列表输出到标准输出
...
rcp其实是remote命令的alias, 需要在你的~/.bashrc中添加:
```
alias rcp='remote -c'
```
